...
function setStatusOverride(version, status, note) {
  const db = read()
  if (!db.statusOv) db.statusOv = {}
  db.statusOv[version] = { status, note: note || '' }
  write(db)
  return db.statusOv[version]
}
...
/* ---- 机型级订阅：db.modelSubs = { [modelId]: { [openid]: { quota, createdAt, updatedAt } } } ---- */

function addModelSub(openid, modelId, count = 1) {
  const db = read()
  const now = Date.now()
  if (!db.modelSubs) db.modelSubs = {}
  if (!db.modelSubs[modelId]) db.modelSubs[modelId] = {}
  const row = db.modelSubs[modelId][openid] || { quota: 0, createdAt: now }
  row.quota = (row.quota || 0) + count
  row.updatedAt = now
  db.modelSubs[modelId][openid] = row
  write(db)
  return { openid, modelId, quota: row.quota }
}

function listModelSubs(modelId) {
  const db = read()
  const m = (db.modelSubs || {})[modelId] || {}
  return Object.keys(m)
    .filter((openid) => (m[openid].quota || 0) > 0)
    .map((openid) => ({ openid, quota: m[openid].quota }))
}

function consumeModelSub(openid, modelId) {
  const db = read()
  const row = (db.modelSubs || {})[modelId] && db.modelSubs[modelId][openid]
  if (!row) return
  row.quota = Math.max(0, (row.quota || 0) - 1)
  row.updatedAt = Date.now()
  write(db)
}

function clearModelSub(openid, modelId) {
  const db = read()
  const row = (db.modelSubs || {})[modelId] && db.modelSubs[modelId][openid]
  if (!row) return
  row.quota = 0
