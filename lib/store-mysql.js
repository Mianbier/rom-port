...
function rowToRom(r) {
  return {
    ...
    android: r.android || '', release: r.release || '', aspatch: r.aspatch || '',
...
function rowToPort(r) {
  return {
    ...
    source: r.source || 'manual', panFileId: r.pan_file_id || '',
    release: r.release || '', createdAt: Number(r.created_at) || 0
  }
}
    ...
    await pool.query(
      `INSERT INTO roms (id, model_id, version, branch, branch_tag, region, android, release, aspatch, recovery, fastboot, manual)
       ...
       ON DUPLICATE KEY UPDATE
         ...
         release=VALUES(release), aspatch=VALUES(aspatch),
    ...
    await pool.query(
      `INSERT INTO ports (id, model_id, version, title, content, size, url, share_url, share_code, source, pan_file_id, release, created_at)
       ...
       ON DUPLICATE KEY UPDATE
         ...
         source=VALUES(source), pan_file_id=VALUES(pan_file_id),
         release=VALUES(release), created_at=VALUES(created_at)`,
      [p.id, p.modelId, p.version, p.title, p.content, p.size, p.url, p.shareUrl, p.shareCode,
       p.source || 'manual', p.panFileId, p.release, p.createdAt || Date.now()]
    )
    return p
  })
}
      ...
      for (const p of list) {
        ...
        await conn.query(
          ...
          [p.id,
           ...
           p.source || 'pan123',
           p.panFileId, p.release, p.createdAt || Date.now()]
        )
      }
      // 记最后同步时间
      await conn.query(
...
async function write() {
  ...
  // 请用 scripts/migrate-to-mysql.js 做一次性导入。
  throw new Error('store-mysql 不支持 write() 整体覆盖；请用 migrate-to-mysql.js')
}
